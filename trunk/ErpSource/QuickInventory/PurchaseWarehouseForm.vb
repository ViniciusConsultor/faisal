...
Public Class PurchaseWarehouseForm
...
#Region "Methods"
  Private Function IsValid() As Boolean
    Try
      Me.PurchaseDetailSpread.EditMode = False
  ...
  Protected Overrides Function SaveRecord() As Boolean
    Try
      ...
      Me.PurchaseDetailSpread.EditMode = False
      Me.PurchaseDetailSpread_Sheet1.SetActiveCell(Me.PurchaseDetailSpread_Sheet1.RowCount - 1, enInventoryColumns.Item_Code)  'For some unknown reason new version of farpoint is not working without this line.
      ...
      If _CurrentInventoryDataRow Is Nothing Then
        ...
        Me.PurchaseNoTextBox.Text = InventoryDataRow.Inventory_No.ToString
      ...
      With _CurrentInventoryDataRow
        ...
        .Party_ID = Me.PartyComboBox.PartyID
        .Payment_Mode = 0
        .Remarks = Me.RemarksTextBox.Text
      ...
      With PurchaseDetailSpread_Sheet1
  ...
  Protected Overrides Function ShowRecord() As Boolean
    Try
      ...
      If Me._InventoryDataTable.Rows.Count > 0 Then
        ...
        Me.PurchaseNoTextBox.Text = Me._CurrentInventoryDataRow.Inventory_No.ToString
        ...
        Me.PartyComboBox.PartyID = Me._CurrentInventoryDataRow.Party_ID
        ...
        Me.PurchaseDetailSpread.SetItemCodeColumns(DirectCast(_InventoryDetailDataTable, DataTable))
        ...
        '  End If
        'End If
        Me.PurchaseDetailSpread_Sheet1.DataSource = Me._InventoryDetailDataTable
        Me.PurchaseDetailSpread.ShowDeleteRowButton(Me.PurchaseDetailSpread_Sheet1) = True
        ...
        Me.PurchaseDetailSpread_Sheet1.RecalculateTotals(enInventoryColumns.Qty01)
        ShowTotal()
        Me.PurchaseDetailSpread.RefreshAllItemCodeAndDescriptions(False)
  ...
  Private Sub ShowTotal()
    ...
    Try
      For I As Int32 = 0 To Me.PurchaseDetailSpread_Sheet1.Rows.Count - 1
        If Not Me.PurchaseDetailSpread_Sheet1.IsRowDeleted(I) Then
          TotalAmount += Val(Me.PurchaseDetailSpread_Sheet1.GetValue(I, enInventoryColumns.TotalAmount + General.ItemCodeColumnsCount))
          TotalQty += Val(Me.PurchaseDetailSpread_Sheet1.GetValue(I, enInventoryColumns.TotalQty + General.ItemCodeColumnsCount))
        Else
  ...
  Private Sub SetGridLayout()
    Try
      ...
      'If PurchaseDetailSpread_Sheet1.ColumnCount < (_InventoryDetailDataTable.Columns.Count + _ItemCodeColumnsCount) Then
      '  Me.PurchaseDetailSpread_Sheet1.Columns.Add(0, _ItemCodeColumnsCount)
      '  For I As Int32 = 0 To _ItemCodeColumnsCount - 1
      '    Me.PurchaseDetailSpread_Sheet1.Columns(I).Label = DatabaseCache.GetItemLeveling(I).Description
      '  Next
      'End If
      Me.PurchaseDetailSpread.ShowDeleteRowButton(Me.PurchaseDetailSpread_Sheet1) = True

      For Each SheetColumn As FarPoint.Win.Spread.Column In Me.PurchaseDetailSpread_Sheet1.Columns
...
#Region "Toolbar methods"
  ...
  Protected Overrides Sub CancelButtonClick(ByVal sender As Object, ByVal e As System.EventArgs)
    Try
      ...
      AddItem()
      SetGridLayout()
      'If Me.PurchaseDetailSpread.Sheets(0).DataSource Is Nothing Then MessageBox.Show("it is nothing")
      'Me.PurchaseDetailSpread.Sheets(0).DataSource = _InventoryDetailDataTable
    Catch ex As Exception
...
#Region "Events"
  ...
  Private Sub PurchaseDetailSpread_EditModeOff(ByVal sender As Object, ByVal e As System.EventArgs) Handles PurchaseDetailSpread.EditModeOff
    Try
      'Dim RowNumber As Int32
      'Dim ColumnNumber As Int32
      'Dim _ItemID As Int32

      If PurchaseDetailSpread.ActiveSheet Is Nothing OrElse PurchaseDetailSpread.ActiveSheet.ActiveCell Is Nothing Then Exit Sub

      'RowNumber = PurchaseDetailSpread.Sheets(0).ActiveCell.Row.Index
      'ColumnNumber = PurchaseDetailSpread.Sheets(0).ActiveCell.Column.Index
      ...
      '  If Int32.TryParse(PurchaseDetailSpread_Sheet1.GetText(RowNumber, enInventoryColumns.Item_ID), _ItemID) Then
      '    Me.StockBarSpread.ShowSummary(Me.LoginInfoObject.CompanyID, _ItemID, 0, 0)
      '  End If
      'End If
      ...
      If PurchaseDetailSpread.ActiveSheet.ActiveRowIndex = PurchaseDetailSpread.ActiveSheet.RowCount - 1 Then
        If PurchaseDetailSpread.ActiveSheet.GetText(PurchaseDetailSpread.ActiveSheet.ActiveRowIndex, enInventoryColumns.Item_Code) <> "" Then
          AddItem()
        End If
      End If

    Catch ex As Exception
      Dim objEx As New QuickExceptionAdvanced("Exception in EditModeOff event of PurchaseDetailSpread.", ex)
  ...
  Private Sub frmSalesInvoice_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    Try
      ...
      'AddAmountColumn()
      With Me.PurchaseDetailSpread
        .LoginInfoObject = Me.LoginInfoObject
        .ItemSheetView = Me.PurchaseDetailSpread_Sheet1
        ...
        .ItemSummaryBarObject = Me.StockBarSpread
      End With
      ...
      'Me.PurchaseDetailSpread.SetItemCodeColumns()

      Me.PartyComboBox.EntityType = Constants.EntityTypes.PurchaseSource
      Me.PartyComboBox.LoadParties(Me.LoginInfoObject.CompanyID)
  ...
  Private Sub DiscountTextBox_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    Try
      ShowTotal()
  ...
  Private Sub SalesTaxTextBox_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    Try
      ShowTotal()
...
#End Region
  ...
  Public Sub New()
    ...
    Me.DocumentType = enuDocumentType.PurchaseWarehouse
