...
#Region "Events"
  Private Sub OkButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles OkButton.Click
    Try
      ...
      If _SQLConnectionObject.State = ConnectionState.Open Then
        'Configuration.ConfigurationManager.ConnectionStrings("Quick_Erp").ConnectionString = GetConnectionString()
        'Configuration.ConfigurationManager.ConnectionStrings.Remove("Quick_Erp")

        ' Declare a FileInfo object for the config file.
        Dim FileInfo As System.IO.FileInfo = New System.IO.FileInfo(My.Application.Info.DirectoryPath & "\" & My.Application.Info.AssemblyName & ".exe.config")
        ...
        For Each Node In XmlDocument.Item("configuration").Item("connectionStrings")
          ...
          If Node.Name = "add" Then
            If Node.Attributes.GetNamedItem("name").Value.ToLower = "quick_erp" Then
              Node.Attributes.GetNamedItem("connectionString").Value = GetConnectionString()
            End If
          End If
        Next Node
        ...
        General.ConfigurationWrite(Constants.CONFIG_KEY_CONNECTION_STRING, GetConnectionString()) 'This will save the connection string to custom configuration file.
        QuickDAL.SharedSetting.QuickErpConnectionString = GetConnectionString() 'This will set the connection string which will be used by all table adapters.
        My.Settings.Reload()

        '_SQLConnectionObject.Close()
        Me.DialogResult = Windows.Forms.DialogResult.OK
        'MessageBox.Show("Connection is successfully established with database, please start the application again", "Restart Application", MessageBoxButtons.OK, MessageBoxIcon.Information)
        'End
        'Now we don't need to close the application because custom configuration file is used for connection string 
        'and shared property will keep current connection string.
  ...
  Private Sub DBConnectivityForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    Try
      ...
      _ConnectionStringBuilderObject.ConnectionString = Configuration.ConfigurationManager.ConnectionStrings("Quick_Erp").ConnectionString
